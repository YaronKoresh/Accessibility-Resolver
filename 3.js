// File 3: ar_utils_dom.js

// --- Accessible Name Check Utilities ---
function ar_hasAriaLabel(el) { return el.getAttribute('aria-label') && el.getAttribute('aria-label').trim(); }
function ar_hasAriaLabelledby(el) {
	const labelledbyAttr = el.getAttribute('aria-labelledby');
	if (!labelledbyAttr) return false;
	return labelledbyAttr.split(/\s+/).some(id => {
		const lblEl = document.getElementById(id);
		return lblEl && (lblEl.textContent || '').trim();
	});
}
function ar_hasTextOrValue(el) { return (el.textContent || '').trim() || (el.value || '').trim(); }
function ar_hasImageWithAltInAnchor(el) {
	const img = el.querySelector('img[alt]');
	return img && (img.alt || '').trim();
}
function ar_hasImageAlt(el) { return (el.alt || '').trim(); }
...
function ar_hasTitleAttr(el) { return !!(el.getAttribute('title') && el.getAttribute('title').trim()); }
...
function ar_hasAccessibleNameForElement(element) {
	...
	if (ar_hasAriaLabel(element) || ar_hasAriaLabelledby(element)) return true;
	const tagName = element.tagName.toUpperCase();
	if ((tagName === 'BUTTON' || (tagName === 'INPUT' && ['submit', 'reset', 'button'].includes(element.type))) && ar_hasTextOrValue(element)) return true;
	if (tagName === 'A' && (ar_hasTextOrValue(element) || ar_hasImageWithAltInAnchor(element))) return true;
	if (tagName === 'IMG' && ar_hasImageAlt(element)) return true;
	if (tagName === 'INPUT' && element.type === 'image' && ar_hasImageAlt(element)) return true;
	if (tagName === 'FIGURE' && ar_hasFigcaption(element)) return true;
	if (['INPUT', 'SELECT', 'TEXTAREA'].includes(tagName) && (ar_hasExplicitLabel(element) || ar_hasParentLabel(element))) return true;
	return ar_hasTitleAttr(element);
}

// --- Auto-fix Wrappers (DOM Manipulation) ---
/**
 * Sets an attribute and logs.
 */
...
// --- Accessibility Menu Helper Functions (DOM related) ---
/**
 * Stores the original inline style value of a CSS property for an element.
 */
...
/**
 * Restores the original inline style value of a CSS property for an element.
 */
...
/**
 * Gets all elements that should be affected by text style adjustments from the menu.
 */
