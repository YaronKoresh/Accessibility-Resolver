...
// before other files attempt to attach methods to them.
var AR_AccessibilityMenu = AR_AccessibilityMenu || {};
var AR_CheckModules = AR_CheckModules || {};


// --- Main Scan and Initialization ---

/**
 * Sets up a mutation observer for live contrast checking.
 */
...
/**
 * Runs the full accessibility scan.
 */
async function ar_runAccessibilityScan() {
	...
	['Critical', 'Moderate', 'Minor', 'Info'].forEach(severity => {
		const issues = unresolvedIssues.filter(r => r.severity === severity && r.wcagLevel !== 'User');
		if (issues.length > 0) {
			const colorMap = { 'Critical': '#c62828', 'Moderate': '#ef6c00', 'Minor': '#0277bd', 'Info': '#546e7a' };
			...
			issues.forEach(issue => {
				...
				if (issue.element instanceof HTMLElement) console.log('%cElement:', 'font-style:italic;', issue.element);
				if (issue.recommendation) console.log(`%cRecommendation: %c${issue.recommendation}`, 'font-style:italic;color:#01579b;', '');
				if (issue.wcagGuideline) console.log(`%cWCAG: %c${issue.wcagGuideline} (${issue.wcagLevel})`, 'font-style:italic;color:#311b92;', '');
				console.log('---');
			});
			console.groupEnd();
		}
	...
	generalObserver.observe(docElement, {
		...
		attributeFilter: ['style', 'class', 'alt', 'href', 'role', 'tabindex', 'aria-hidden', 'aria-label', 'id', 'for', 'value', 'src', 'lang', 'title']
	});
}

/**
 * Initializes the accessibility menu and runs the scan.
 */
function ar_initializeAndRunMerged() {
    ...
    // Ensure AR_AccessibilityMenu and AR_CheckModules are fully populated before calling init/scan
    // This is more of a safeguard if file loading order is not guaranteed.
    // The IIFE structure in other files should handle method attachment.
    if (typeof AR_AccessibilityMenu.init !== 'function') {
     ...
     if (typeof AR_CheckModules.checkDocumentGlobals !== 'function') { // Check one of the methods
